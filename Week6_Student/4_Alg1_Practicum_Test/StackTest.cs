...
namespace Alg1_Practicum_Test
{
    ...
    public class StackTest : ContextBoundObject
    {
        ...
        [TestMethod]
        [AantalPuntenAlsSlaagt(1)]
        [PrintCode("Stack", "Peek")]
        [TestCategory("WS6 - Stack")]
        [TestSummary(@"We maken een lege Stack aan en roepen peek() aan. Dit zou null moeten teruggeven.")]
        public void Stack_Peek_Empty_ShouldReturnNull()
        {
            Assert.IsNull(stack.Peek(), "\n\nStack.peek(): Een lege stack moet null teruggeven.");
        }
        ...
        public void Stack_PushPeek_ShouldReturnPushed()
        {
            stack.Push("test");

            Assert.AreEqual("test", stack.Peek(), "\n\nStack.peek(): Een stack peek() moet teruggeven wat met push() is gegeven.");
        }
        ...
        [PrintCode("Stack", "Peek")]
        [TestCategory("WS6 - Stack")]
        [TestSummary(@"We maken een Stack aan en roepen deze aan met push(""test""). Door de aanroep van peek() mag deze waarde niet van de stack verdwijnen.")]
        public void Stack_PushPeek_ShouldRetainPushedItem()
        {
            
            stack.Push("test");
            stack.Peek();

            Assert.AreEqual("test", stack.Pop(), "\n\nStack.peek(): Een stack peek() mag het item niet van de stack verwijderen.");
        }
